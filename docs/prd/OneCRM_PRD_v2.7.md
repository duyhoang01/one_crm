...
[6.7.8. FR-INT-07: Channel Health Dashboard (Admin) [125](#fr-int-07-channel-health-dashboard-admin)](#fr-int-07-channel-health-dashboard-admin)
...
| runtime_config | jsonb | ✓ | Cấu hình vận hành **theo từng sản phẩm** — mọi ngưỡng của M-05 đọc từ đây, **không hardcode**:<br>· `webhook_secret`<br>· `usage_push_interval` — chu kỳ sản phẩm đẩy mức sử dụng (**chốt v2.6: 3h** cho mọi sản phẩm). Ngưỡng "chưa cập nhật" = **2 × giá trị này**<br>· `mismatch_sla_minutes` — ngưỡng lệch trạng thái (**chốt v2.6: 15 phút** cho mọi sản phẩm)<br>· `auto_reconcile_after_hours` — ngưỡng "chờ kích hoạt quá lâu". **Để trống → KHÔNG kiểm tra** (EDR: triển khai 1–2 tháng là bình thường)<br>· `renewal_notice_days` — báo gia hạn trước bao nhiêu ngày (**chốt v2.6: 60**)<br>· `upsell_threshold_pct` — ngưỡng "sắp dùng hết" để chào bán thêm (**chốt v2.6: 90%**)<br>· `supports_force_sync` bool — false thì CRM **ẩn** nút Đồng bộ lại<br>· `portal_url_template` — dạng `https://.../{external_ref}`; null thì không hiển thị deep-link |
...
| activation_date | date | optional | Ngày license active thực (từ webhook license.activated). Nguồn cho sub.activated_at |
...
| last_event_occurred_at | timestamp | optional | `occurred_at` của sự kiện license **mới nhất đã áp dụng** — mốc **chống sự kiện đến muộn** (UC-INT-02 BR-05: chỉ áp dụng khi tin mới hơn mốc này). Cũng là nền của **xử lý lại theo chuỗi** (UC-INT-05) |
| last_change_source | enum (command_ack / product_event) | optional | Nguồn của lần đổi trạng thái gần nhất: do **lệnh CRM** (command-ack, tin có `correlation_id`) hay **tự nhiên** từ sản phẩm (UC-INT-02 BR-12) |
| source_command_event_id | UUID | optional | Khi `last_change_source=command_ack`: `event_id` của lệnh CRM đã gây ra thay đổi (nối về `OUTBOX_EVENT.correlation_id`) |
| metadata | jsonb | optional | Forward-compat |
...
| Field | Type | Required | Mô tả |
|----|----|----|----|
| id | UUID | ✓ | PK (= `event_id` trên dây) |
| subscription_id | UUID FK | ✓ | Subscription mà lệnh tác động — **khoá map về sub** *(SG-3, tách khỏi correlation_id)* |
| product_id | UUID FK | ✓ | Sản phẩm đích — **denormalize** để lọc & gộp theo sản phẩm ở UC-INT-03/04 mà không phải join sub→package→product *(SG-2)* |
| event_key | string | ✓ | Loại lệnh, vd "subscription.suspended" (đồng bộ tên với metadata §6.7.1 — trước là `event_type`) |
| correlation_id | UUID | ✓ | **= `event_id` của lệnh (product echo lại → nối lệnh ↔ phản hồi command-ack). KHÔNG phải subscription_id** *(chốt SG-3 16/07)* |
...
| subscription_id | UUID FK | ✓ | **Khoá map về subscription** *(SG-3, tách khỏi correlation_id)* |
| correlation_id | UUID | optional | **= `event_id` của lệnh khi tin là phản hồi lệnh (command-ack, §3.3); trống nếu event tự nhiên. KHÔNG phải sub_id** *(chốt SG-3 16/07)* |
| event_key | string | ✓ | Loại sự kiện (đồng bộ tên với metadata §6.7.1 — trước là `event_type`) |
| occurred_at | timestamp | ✓ | Thời điểm sự việc **thật sự xảy ra ở nguồn** — nền của **chống đến muộn** (UC-INT-02 BR-05) & **xử lý lại theo chuỗi** (UC-INT-05 BR-03/BR-08 sắp theo `occurred_at`) |
| schema_version | string | optional | Version payload theo metadata chuẩn (§6.7.1) |
...
> **SG-4 (chốt 16/07) — chữ ký KHÔNG lưu vào bảng này.** Receiver verify HMAC tại ingress (chấp nhận khoá cũ + mới trong 7 ngày ân hạn — BR-02). Tin **sai chữ ký → trả 4xx, KHÔNG ghi `WEBHOOK_INBOX`** (chỉ log hạ tầng rate-limited kèm source IP cho forensics — ngoài phạm vi CRM DB). Do đó **mọi dòng trong bảng đều đã xác thực** → bỏ cột `signature` / `signature_ok`. Sự cố khoá sai được **cảnh báo IM LẶNG** bắt (sản phẩm sai khoá → không tin nào hợp lệ → kênh im lặng).
...
DRAFT → PENDING_PROVISION → ACTIVE ──┬── webhook license.suspended (tự nhiên: hết ân hạn, không gia hạn) ──► SUSPENDED\
├── Sales: suspend ──────────────► SUSPENDED\
├── Manager: revoke ─────────────► REVOKED (terminal)\
├── successor ACTIVE ────────────► RENEWED (terminal)\
└── webhook: license EXPIRED* ───► EXPIRED   (* chỉ product có "hard-expiry"; **EDR KHÔNG dùng** — xem Lưu ý)\
\
...
Ví dụ EDR: PENDING / PROVISIONING / ACTIVE / EXPIRING_SOON / GRACE / RESTRICTED / EXPIRED / SUSPENDED / REVOKED / RENEWED.
...
| Active thực | license_mirror.activation_date → sub.activated_at | Webhook license.activated | Ngày bắt đầu tính hiệu lực thực |
| Hết hiệu lực thực | license_mirror.expiration_date → đồng bộ sub.end_date | Webhook license.activated (product) | Nguồn sự thật cho hạn & renewal |

**Đồng bộ khi license.activated về:**
...
created ──► \[provisioning/UAT\] ──► activation_date ──┤ thời hạn license ├── expiration_date ──► \[ân hạn\] ──► \[hạn chế\] ──► SUSPENDED\
(DRAFT) (PENDING_PROVISION) (sub ACTIVE) (= sub.end_date) (mirror GRACE, sub ACTIVE) (mirror RESTRICTED, sub ACTIVE) (webhook license.suspended)

- Sub sống trước activation (giai đoạn provisioning) và sống sau end_date (ân hạn + hạn chế) → tổng thời gian sub \> thời hạn license.

- end_date = ngày license hết hiệu lực (KHÔNG cộng cứng grace). **Mô hình EDR (xác nhận 16/07):** sau end_date, product push `license.expired` → mirror **GRACE** (sub vẫn ACTIVE) → `license.grace_expired` → mirror **RESTRICTED** (sub vẫn ACTIVE) → `license.suspended` → sub **SUSPENDED** — tất cả qua FR-SUB-11. `sub.status` giữ ACTIVE suốt ân hạn/hạn chế, **không** đi qua EXPIRED.
...
| **P4** | `sub.status` (CRM) và license thật (product) lệch nhau | CRM báo "Active" trong khi KH mất dịch vụ — hoặc ngược lại, thất thoát | **Không tự đặt ACTIVE khi Sales bấm** — chờ webhook `license.activated`; **auto-transition** theo license state (FR-SUB-11) |
...
**Kích hoạt license — actor cấu hình.** Đưa sub sang ACTIVE tách **trigger** khỏi **execute/confirm**. Actor trigger cấu hình per Product Module/deal (AUTO\|PRODUCT_ADMIN\|CUSTOMER_SELF\|SALES); execute & xác nhận luôn do product qua webhook `license.activated` (air-gap mới xác nhận thủ công). Khi actor = SALES (vd EDR sau UAT — cắt khâu Sales báo product để admin bấm): Sales bấm "Kích hoạt" → CRM publish intent `license.activation_requested` → product active → webhook `license.activated` → sub ACTIVE. **CRM không tự đặt ACTIVE lúc Sales bấm** (chờ webhook, tránh lệch `sub.status` ↔ `license_mirror.status`); air-gap mới flip thủ công. "Kích hoạt" là capability có audit; RBAC gán theo cấu hình vận hành, không hardcode.
...
- **FR-SUB-07 (Gia hạn / UC-SUB-07):** sub kế tiếp tạo **trực tiếp ACTIVE** (bỏ mô hình DRAFT → submit của v2.3), predecessor → **RENEWED ngay**; license_mirror vẫn theo webhook inbound từ product (xem §5.5.1). *Lưu ý:* trong khoảng chờ webhook, sub ACTIVE nhưng mirror còn PENDING — **FR-LIC-07 cần loại trừ sub vừa gia hạn** khỏi cảnh báo mismatch cho tới khi `license.activated` về.
...
| **Business Rules** | BR-11.1: Mapping declare per Product Module. **EDR (xác nhận 16/07):** `license.activated` → sub ACTIVE (từ PENDING_PROVISION); `license.expired` → mirror GRACE, **sub giữ ACTIVE**; `license.grace_expired` → mirror RESTRICTED, **sub giữ ACTIVE**; `license.suspended` → **sub SUSPENDED**; `license.reactivated` → sub ACTIVE. *(EDR không map sang sub EXPIRED.)*BR-11.2: Không auto-transition nếu mapping không declareBR-11.3: Auto-transition: append PROVISIONING_EVENT + auditBR-11.4: Race condition: sub đã terminal khác → bỏ qua |
| **User Stories** | US-12: Là hệ thống, tôi tự đồng bộ trạng thái sub theo license để Sales/CSKH không phải cập nhật tay |
| **AC** | AC-11.1: `license.activated` lần đầu → sub → ACTIVEAC-11.2: `license.expired` → mirror GRACE, sub **giữ ACTIVE** (không sang EXPIRED)AC-11.3: `license.suspended` (tự nhiên) → sub → SUSPENDEDAC-11.4: Sub REVOKED, webhook đến sau → bỏ qua |
| **API** |  |
...
2. **Sub vừa gia hạn: ACTIVE + mirror PENDING là hợp lệ tạm thời.** Từ v2.4, sub kế tiếp tạo **thẳng ACTIVE**, còn license mirror chờ webhook `license.activated`. Khoảng chờ này là **mismatch được chấp nhận** — FR-LIC-07 **phải loại trừ**, nếu không mỗi lần gia hạn sẽ đẻ ra một cảnh báo giả.
...
| Mô tả | Worker M-07 gọi khi webhook event=license.usage_synced. Overwrite usage_snapshot. |
|----|----|
| **Actor** | System |
...
| **Output** |  |
| **Business Rules** | BR-02.1: Append-onlyBR-02.2: event_type lạ → append với category="unknown" + audit warningBR-02.3: Display message render tại view-timeBR-02.4: Idempotent qua source_webhook_idBR-02.5: Skip event_type tần suất cao (vd license.usage_synced) trừ khi Product Module declare emit_timeline=true |
| **User Stories** | US-04: Là hệ thống, tôi ghi tiến độ provisioning vào timeline để Sales/CSKH theo dõi |
| **AC** | AC-02.1: Append tenant.created → row mớiAC-02.2: event_type lạ → category="unknown" + warningAC-02.3: Duplicate source_webhook_id → no-opAC-02.4: license.usage_synced không emit → skip |
| **API** |  |
...
OneCRM là **bên quan sát** (passive-observer, YT-1/YT-3): nó **không gọi API sản phẩm trực tiếp**, mà trao đổi với mọi Product Module (EDR, C-Shield, AV, CA…) **hoàn toàn bằng sự kiện** — gửi đi qua Outbox + hàng đợi, nhận về qua webhook. M-07 là **lớp đường ống (plumbing)** hiện thực đúng ranh giới đó cho toàn hệ thống: một cơ chế **gửi/nhận sự kiện đáng tin cậy** (không mất, không trùng, không sai thứ tự) dùng chung cho mọi sản phẩm — thêm sản phẩm mới chỉ khai bảng ánh xạ, **không sửa lõi**.

Nếu không có M-07 (hoặc làm ẩu): lệnh CRM gửi đi có thể **mất mà không ai biết**; sự kiện sản phẩm báo về có thể **đến sai thứ tự → license sai trạng thái vĩnh viễn**; tin giả mạo có thể đổ rác vào DB. M-07 tồn tại để **những sự cố đó không xảy ra**, và khi có trục trặc tích hợp thì **có màn để nhìn ra và xử lý lại**.

#### b) Ai dùng, khi nào, để làm gì

| Nhóm | Vào từ đâu | Khi nào | Để làm gì |
|---|---|---|---|
| **System (worker nền)** — người dùng chính | Outbox Publisher · Webhook Receiver + Processor | Liên tục (event-driven; usage theo lịch **3h**) | Đẩy lệnh đi & xử lý tin về theo đúng ranh giới YT-3 — **tính năng ngầm, không giao diện** (UC-INT-01, UC-INT-02) |
| **License Admin** — vận hành tích hợp | Menu **Tích hợp** → 4 tab | Khi có cảnh báo / Sales báo "khách chưa dùng được" | Xem tình trạng kết nối, gửi lại lệnh kẹt, xử lý lại tin lỗi, tra cứu một subscription (UC-INT-03/04/05/06) |
| **Auditor** | Menu Tích hợp *(chỉ đọc)* | Khi soát | Xem đầy đủ, **không** có nút gửi lại / xử lý lại |
| **Sản phẩm** *(external)* | Webhook endpoint | Khi trạng thái license đổi / báo mức dùng / báo không thực thi được lệnh | Nguồn **duy nhất** làm trạng thái license trong CRM thay đổi (UC-INT-02) |

UC-INT-01/02 là đường ống chạy nền liên tục; UC-INT-03..06 là 4 tab vận hành (License Admin dùng khi có sự cố, không thường trực).

#### c) 4 pain point mà module này sinh ra để giải

| # | Pain point | Hậu quả nếu không có M-07 | Giải bằng |
|---|---|---|---|
| **P1** | Lệnh CRM gửi đi bị mất do hàng đợi/hạ tầng chết, nhưng nghiệp vụ đã ghi | Khách bị tính đã dừng/đã đổi trong CRM mà sản phẩm **chưa hề nhận lệnh** — mất lệnh mà **không ai biết là đã mất** | **Transactional Outbox** (ghi sự kiện cùng transaction nghiệp vụ) + worker retry + "Đã ngừng tự gửi lại" hiện ở UC-INT-04 để gửi lại tay (UC-INT-01, UC-INT-04) |
| **P2** | Sự kiện sản phẩm báo về **đến sai thứ tự** (webhook có retry) | Tin cũ ghi đè tin mới → **license sai trạng thái vĩnh viễn** (khách đã dừng vẫn hiện "đang hoạt động") | **Chống đến muộn** theo `occurred_at` (UC-INT-02 BR-05) + **xử lý lại theo chuỗi** dựng lại đúng trình tự (UC-INT-05) |
| **P3** | Không phân biệt được "sản phẩm không gửi gì" (lỗi phía sản phẩm) với "có gửi nhưng CRM xử lý lỗi" | Đổ lỗi nhầm, sửa nhầm chỗ, tốn thời gian điều tra | **Dashboard tình trạng kết nối hai chiều** tô đỏ theo ngưỡng + **dòng thời gian gộp một subscription** (UC-INT-03, UC-INT-06) |
| **P4** | Tin giả mạo / sai khoá đổ rác vào inbox → phình bảng, DoS, dữ liệu không đáng tin | Inbox lẫn tin chưa xác thực; điều tra tấn công lẫn vào việc vận hành CRM | **Xác thực HMAC tại ingress, tin sai chữ ký trả 4xx & KHÔNG ghi inbox** (SG-4) → mọi dòng trong DB đều đã xác thực (UC-INT-02 BR-01) |

#### d) Hợp đồng metadata webhook (Core — mọi sản phẩm)

Webhook inbound — CRM định nghĩa 2 tầng bắt buộc:

**Lớp 1 — Metadata (Core, mọi product):**
...
| Body | event_key | Chọn mapping | 400 |
| Body | occurred_at | Thời điểm sự kiện thật ở nguồn (chống đến muộn) | 400 |
| Body | subscription_id | Khoá đối chiếu về subscription | 400 |
| Body | product_id | Chọn khoá bảo mật + bảng ánh xạ đúng sản phẩm | 400 |
| Body | correlation_id | Nối lệnh CRM ↔ phản hồi sản phẩm | bắt buộc với phản hồi lệnh; vắng → xử lý như event tự nhiên |
| Body | schema_version | Versioning payload | 400 |
| Body | payload | Nội dung tùy event | tùy event |

> **Đồng bộ 16/07/2026 theo `_event_catalog.md` §2 (nguồn sự thật):** đổi `event_type`→`event_key`, `data`→`payload`; bổ sung `subscription_id` + `product_id` là trường riêng; `schema_version` thành bắt buộc.

`subscription_id` là khoá chính đối chiếu về subscription của CRM; fallback tra `license_mirror.external_ref`. `correlation_id` chỉ dùng để nối phản hồi inbound với đúng lệnh outbound đã gửi (command-ack, §3.3), KHÔNG phải khoá đối chiếu subscription.

**Lớp 2 — Payload bắt buộc theo event_key:** mỗi Product Module khai báo "required fields" trong inbound_event_mapping (vd EDR: license.activated cần activation_date+expiration_date; status-change cần status raw; license.usage_synced theo usage_snapshot_schema).

Validate (transport vs nội dung): **HMAC sai → trả 4xx, KHÔNG ghi WEBHOOK_INBOX** (chỉ log hạ tầng — SG-4); thiếu metadata / replay → reject ở Receiver (4xx); đủ metadata + chữ ký hợp lệ nhưng thiếu payload bắt buộc theo event → ghi WEBHOOK_INBOX (idempotent) → Processor đánh FAILED + alert (fail-and-alert); subscription_id không map được sub → FAILED + alert (orphan).

#### e) Đặc tả chi tiết — Danh sách FRS (UC-INT)

> Hành vi nghiệp vụ chi tiết của M-07 được chốt trong bộ **FRS UC-INT-01..06** (`docs/frs/integration/`) — mỗi FRS gồm Thông tin chung + Business Rule + Luồng nghiệp vụ (kèm sơ đồ BPMN) + Mô tả giao diện + Acceptance Criteria. Khi FR-INT (PRD) và FRS lệch nhau, **FRS là nguồn chốt** (đã đồng bộ demo `v2.7.0_integration` chốt 16/07). Danh mục sự kiện hai chiều: [_event_catalog.md](../frs/integration/_event_catalog.md).

| UC-INT | Tên | Ngầm/UI | FR-INT (PRD) | Tài liệu chi tiết |
|---|---|---|---|---|
| UC-INT-01 | Gửi sự kiện đi *(Publish Outbound)* | Ngầm | FR-INT-01, FR-INT-02 | [UC-INT-01](../frs/integration/UC-INT-01_publish_outbound_event.md) |
| UC-INT-02 | Tiếp nhận & xử lý sự kiện từ sản phẩm *(Inbound)* | Ngầm | FR-INT-03, FR-INT-04 | [UC-INT-02](../frs/integration/UC-INT-02_receive_inbound_event.md) |
| UC-INT-03 | Xem tình trạng kết nối *(Channel Health)* | UI | **FR-INT-07** | [UC-INT-03](../frs/integration/UC-INT-03_view_channel_health.md) |
| UC-INT-04 | Sự kiện gửi đi & Gửi lại lệnh | UI | FR-INT-05, FR-INT-06 | [UC-INT-04](../frs/integration/UC-INT-04_outbox_and_retry.md) |
| UC-INT-05 | Sự kiện nhận về & Xử lý lại theo chuỗi | UI | FR-INT-05, FR-INT-06 | [UC-INT-05](../frs/integration/UC-INT-05_inbox_and_replay.md) |
| UC-INT-06 | Tra cứu theo subscription | UI | FR-INT-05 | [UC-INT-06](../frs/integration/UC-INT-06_trace_by_subscription.md) |
...
| **Input** | POST /api/v1/webhook/{product_code}/events<br>Headers: X-Signature, X-Timestamp — Body: event_id, event_key, occurred_at, subscription_id, product_id, correlation_id, schema_version, payload *(đồng bộ metadata §6.7.1.d)* |
| **Output** | HTTP 200 (ack ngay) hoặc 4xx (chữ ký/timestamp/schema) / 404 / 429 theo lỗi verify |
| **Business Rules** | **BR-03.1 *(SG-4)*: Verify HMAC-SHA256; sai chữ ký → trả 4xx và KHÔNG ghi WEBHOOK_INBOX** — chỉ log hạ tầng rate-limited + source IP cho forensics (nhờ vậy mọi dòng trong inbox đều đã xác thực; UC-INT-02 BR-01)BR-03.2: Timestamp window 5 phút → fail 400 STALE_TIMESTAMPBR-03.3: Idempotency: event_id đã có → 200 {status: "duplicate"}BR-03.4: Basic schema/metadata validate → fail 400BR-03.5: Product registered check → 404 PRODUCT_NOT_REGISTEREDBR-03.6: Chữ ký hợp lệ + đủ metadata → ghi WEBHOOK_INBOX status=RECEIVED (kèm occurred_at, subscription_id), return 200 ngayBR-03.7: Secret rotation grace 7 ngày — dual-key (old + new valid)BR-03.8: Rate limit 100 req/s/product → 429 |
| **User Stories** | US-03: Là Product Backend, tôi muốn gửi webhook nhanh không phải đợi CRM process — chỉ verify + ack |
| **AC** | AC-03.1: Valid → 200, WEBHOOK_INBOX tạoAC-03.2: Sai chữ ký → 4xx, KHÔNG ghi WEBHOOK_INBOX (SG-4)AC-03.3: Timestamp \> 5 phút → 400 STALE_TIMESTAMPAC-03.4: Duplicate event_id → 200 {status: "duplicate"}AC-03.5: Unknown product → 404AC-03.6: Rate limit → 429AC-03.7: Secret rotation grace: old → 200 trong 7 ngày |
| **API** |  |
...
| **Business Rules** | BR-04.1: Polling 500msBR-04.2: Apply Inbound Mapping của Product Module:<br>Map event_key → actionValidate payload schemaGọi FR-LIC-02/03 + FR-EVT-02 + FR-SUB-11<br>BR-04.3: Retry exponential, max 5 retries/1hBR-04.4: Max retries → FAILED + alert; không block khácBR-04.5: **Giữ thứ tự per subscription** (tuần tự trong cùng sub — KHÔNG phải per correlation_id)BR-04.6: Idempotent qua event_idBR-04.7: P95 RECEIVED → PROCESSED ≤ 2s<br>**BR-04.8 *(chống đến muộn — UC-INT-02 BR-05)*: chỉ áp dụng khi `occurred_at` mới hơn `license_mirror.last_event_occurred_at`; cũ hơn → ghi nhận + nhãn "đến muộn", KHÔNG áp dụng (tránh ghi đè ngược trạng thái)**BR-04.9 *(command-ack — UC-INT-02 BR-12)*: `license.suspended`/`license.reactivated` mang `correlation_id` = phản hồi lệnh CRM → ghi `last_change_source=command_ack` + `source_command_event_id`; vắng `correlation_id` = tự nhiên (`product_event`, hợp lệ)BR-04.10: trạng thái license chỉ đổi do event sản phẩm (không tự suy diễn); ánh xạ EDR `license.expired`→GRACE, `license.grace_expired`→RESTRICTED, `license.suspended`→SUSPENDED (§6.9.7) |
| **User Stories** | US-04: Là hệ thống, tôi xử lý webhook product để đồng bộ trạng thái vào CRM |
| **AC** | AC-04.1: RECEIVED → PROCESSED \< 2sAC-04.2: Mapping fail → FAILED, audit ghi, không blockAC-04.3: 2 webhook cùng sub → áp dụng theo occurred_at (tin đến muộn không ghi đè tin mới hơn — BR-05)AC-04.4: Crash → instance khác pickup, idempotentAC-04.5: Unknown event_type → timeline category="unknown", warning |
| **API** |  |

### 6.7.6. FR-INT-05: View Outbox & Webhook Inbox (Admin)

| Mô tả | Nền **"xem"** cho vận hành tích hợp: liệt kê & lọc **OUTBOX_EVENT** (tab Sự kiện gửi đi — UC-INT-04), **WEBHOOK_INBOX** (tab Sự kiện nhận về — UC-INT-05), và tra cứu **toàn bộ trao đổi hai chiều của một subscription** trên một dòng thời gian gộp (tab Tra cứu — UC-INT-06). *(Tổng quan tình trạng kết nối = FR-INT-07; hành động gửi lại / xử lý lại = FR-INT-06.)* |
|----|----|
| **Actor** | License Admin *(xem)* · Auditor *(chỉ đọc)* |
| **Input** | filter (status, product, event_key, subscription_id, time_range — **mặc định 3 ngày gần nhất**), phân trang 15 dòng/trang |
| **Output** | list outbox / webhook inbox + total; dòng thời gian gộp theo subscription (gộp sự kiện lặp "× N") |
| **Business Rules** | BR-05.1: Quyền `integration:view` — mặc định License Admin + Auditor (dữ liệu vận hành nhạy cảm; Sales/CSKH/Manager không thấy menu)BR-05.2: **Auditor chỉ đọc** — không có nút gửi lại / xử lý lại ở bất kỳ đâu (hành động thuộc FR-INT-06)BR-05.3: Lọc mặc định 3 ngày gần nhất (đồng bộ UC-INT-04/05 BR-09) — nới thời gian để tra cứu xa hơnBR-05.4: Tra cứu theo `subscription_id` (dòng thời gian hai chiều) + `correlation_id` (nối lệnh ↔ phản hồi) khi Sales báo lỗiBR-05.5: P95 ≤ 1s với 1M events |
| **User Stories** | US-05: Là License Admin, tôi muốn xem lệnh outbox "Đã ngừng tự gửi lại" để biết lệnh nào chưa tới nơiUS-06: Là License Admin, tôi muốn tra toàn bộ dòng thời gian của sub X khi Sales báo "khách chưa dùng được" |
| **AC** | AC-05.1: View outbox DEAD_LETTER → list kèm lý do lỗi cuốiAC-05.2: View inbox "Xử lý lỗi" → list kèm lý do (bước nào + chi tiết)AC-05.3: Tra `subscription_id` → dòng thời gian hai chiều, sự kiện lặp gộp "× N"AC-05.4: Auditor mở màn → xem đầy đủ, không nút hành động nào |
| **API** |  |

### 6.7.7. FR-INT-06: Manual Retry / Reprocess

| Mô tả | Admin **gửi lại lệnh outbox** "Đã ngừng tự gửi lại" (UC-INT-04), hoặc **xử lý lại theo chuỗi** tin inbox lỗi (UC-INT-05), sau khi đã xử lý nguyên nhân. Cần quyền `integration:retry` (Auditor không có). |
|----|----|
| **Actor** | License Admin |
| **Input** | outbox_id (retry) hoặc webhook_inbox_id (reprocess) + reason |
| **Output** | HTTP 200 (đã đưa lại vào hàng đợi) |
| **Business Rules** | **Gửi lại lệnh outbox (UC-INT-04):** BR-06.1: chỉ lệnh **DEAD_LETTER** ("Đã ngừng tự gửi lại") mới gửi lại được — KHÔNG gửi lại lệnh FAILED_RETRYING ("Đang thử lại" — hệ thống đang tự xử lý) hay PUBLISHED ("Đã gửi") (BR-02)BR-06.2: gửi lại = reset PENDING, retry_count=0, **GIỮ NGUYÊN event_id + payload** (không sinh lệnh mới, không sửa nội dung — BR-04, YT-3), giữ đúng thứ tự per subscription (BR-07)BR-06.3: lệnh "Sản phẩm từ chối" (`subscription.command_failed`) gửi lại được nhưng cảnh báo "chỉ có ích sau khi sửa nguyên nhân" (BR-03)<br>**Xử lý lại tin inbox — THEO CHUỖI (UC-INT-05):** BR-06.4: chọn 1 tin lỗi → xử lý lại **cả chuỗi** sự kiện của cùng subscription từ tin đó trở về sau, **tuần tự theo occurred_at** (BR-03) — không xử lý lẻ (xử lý lẻ tin cũ = dựng lại lịch sử sai)BR-06.5: **KHÔNG gửi thông báo cho khách hàng, KHÔNG phát sinh event mới sang sản phẩm** (BR-05); cho **xem trước** chuỗi + trạng thái license sau khi xong (BR-04); chuỗi lỗi giữa chừng → DỪNG ngay tại tin lỗi (EF-04)<br>**Chung:** BR-06.6: bắt buộc nhập lý do + ghi nhật ký (ai · lý do · lúc nào)BR-06.7: idempotent qua event_idBR-06.8: hàng loạt trần **500/lượt**, vượt → báo rõ số bị bỏ lại; Auditor KHÔNG có quyền (BR-01) |
| **User Stories** | US-07: Là Admin, tôi muốn retry/reprocess sau khi fix lỗi để không phải can thiệp DB tay |
| **AC** | AC-06.1: Gửi lại outbox DEAD_LETTER → PENDING, worker pickup; giữ nguyên event_id + payloadAC-06.2: Xử lý lại 1 tin lỗi → chạy lại **cả chuỗi** cùng sub theo occurred_at; trạng thái license về đúng sự kiện cuối chuỗiAC-06.3: Xử lý lại KHÔNG gửi thông báo cho khách, KHÔNG phát event mới sang sản phẩmAC-06.4: Bỏ trống lý do → chặn (bắt buộc nhập)AC-06.5: Chuỗi lỗi giữa chừng → dừng tại tin lỗi, không chạy tiếpAC-06.6: Auditor gọi API retry → 403 |
| **API** |  |

### 6.7.8. FR-INT-07: Channel Health Dashboard (Admin)

> **FR mới** — không có trong PRD v2.6 gốc; đề xuất & chốt theo **UC-INT-03** (đóng OQ-LIC-03: M-05 không tự phân biệt "sản phẩm không gửi gì" vs "CRM xử lý lỗi"). Bản Phase 1 **giản lược**: chỉ tô màu theo ngưỡng + nút hành động nhanh, KHÔNG diễn giải nguyên nhân thành câu (xem UC-INT-03 §5 để hiểu đánh đổi).

| Mô tả | Dashboard mỗi sản phẩm **một thẻ**, tổng hợp tình trạng kết nối **hai chiều** (lệnh CRM gửi có tới nơi không / sản phẩm có báo về không, CRM xử lý được không); số liệu bất thường **tô đỏ** theo ngưỡng cấu hình + nút bấm nhanh sang màn chi tiết khi có việc cần xử lý. |
|----|----|
| **Actor** | License Admin · Auditor *(chỉ đọc)* |
| **Input** | (không) — tổng hợp theo sản phẩm |
| **Output** | Danh sách thẻ sản phẩm: nửa Gửi đi (chờ gửi · đang thử lại · đã ngừng tự gửi lại) + nửa Nhận về (nhận gần nhất · lỗi tồn đọng) |
| **Business Rules** | BR-07.1: Phân quyền `integration:view` — mặc định License Admin + Auditor; Sales/CSKH/Manager KHÔNG thấy menuBR-07.2: mỗi sản phẩm một thẻ 2 nửa; một thẻ có thể tô đỏ cả hai nửa cùng lúc (không loại trừ)BR-07.3: "Đã ngừng tự gửi lại" / "Lỗi tồn đọng" là **SỐ ĐẾM việc chưa xử lý** (tất cả, không giới hạn tuổi) — KHÔNG phải tỷ lệ %, KHÔNG cửa sổ 7 ngày (COUNT theo status, rẻ)BR-07.4: tô đỏ theo ngưỡng; **ngưỡng im lặng = 2 × `runtime_config.usage_push_interval`** (không hardcode, KHÔNG hiện số ngưỡng ra màn)BR-07.5: loại trừ khách triển khai tại chỗ (không có kênh tự động)BR-07.6 *(SG-4)*: **KHÔNG có chỉ số "chữ ký không hợp lệ"** — tin sai chữ ký bị trả 4xx, không ghi inbox → không đếm (UC-INT-02 BR-01)BR-07.7: khi bất thường → nút nhanh sang UC-INT-04 (lọc DEAD_LETTER + sản phẩm) / UC-INT-05 (lọc FAILED + sản phẩm) |
| **User Stories** | US-08: Là License Admin, tôi muốn nhìn một màn biết ngay kênh sản phẩm nào đang trục trặc, chiều nào, để vào xử lý đúng chỗ |
| **AC** | AC-07.1: Im lặng quá ngưỡng (2× chu kỳ) → dòng "Nhận gần nhất" tô đỏ; không hiện số ngưỡngAC-07.2: Lỗi tồn đọng > 0 → tô đỏ + nút "Xem sự kiện nhận về"AC-07.3: Lệnh chưa tới nơi > 0 → tô đỏ + nút "Xem N events chưa tới nơi" (mở UC-INT-04)AC-07.4: Bình thường → nhãn "✔ Bình thường", không nútAC-07.5: Sales gọi API → 403; Auditor xem được, không nút hành độngAC-07.6: Đổi `usage_push_interval` → ngưỡng tự đổi, không sửa code |
| **API** |  |
...
> **Đồng bộ 16/07/2026 theo `docs/frs/integration/_event_catalog.md` §3 (nguồn sự thật).** Bổ sung `EXPIRING_SOON`, `RESTRICTED`, `REVOKED`; sửa nhãn GRACE ("Trong ân hạn", không phải "gia hạn"); tách mô hình ân hạn hai giai đoạn.

| Status        | Display VN            | Sub state mapping                                            |
|---------------|-----------------------|-------------------------------------------------------------|
| PENDING       | "Chờ khởi tạo"        | PENDING_PROVISION                                           |
| PROVISIONING  | "Đang triển khai"     | PENDING_PROVISION                                           |
| ACTIVE        | "Đang hoạt động"      | ACTIVE (từ PENDING_PROVISION)                              |
| EXPIRING_SOON | "Sắp hết hạn"         | ACTIVE giữ nguyên (không transition)                        |
| GRACE         | "Trong ân hạn"        | ACTIVE giữ nguyên — khách vẫn dùng đủ tính năng             |
| RESTRICTED    | "Hạn chế tính năng"   | ACTIVE giữ nguyên *(OQ-INT-08 chốt 16/07 — sub chưa chết; hạn chế 30 ngày)* |
| EXPIRED       | "Hết hạn"             | EXPIRED *(chỉ khi gói KHÔNG có ân hạn — trường hợp biên)*   |
| SUSPENDED     | "Đã ngừng"            | SUSPENDED (nếu chưa SUSPENDED)                              |
| REVOKED       | "Đã thu hồi"          | REVOKED *(⚠️ EDR Phase 1 chưa gửi `license.revoked` thật — UI/cảnh báo phản ứng REVOKED tạm ẩn, §3.4)* |
| RENEWED       | "Đã gia hạn"          | RENEWED (nếu có successor ACTIVE)                          |
...
Display: "145 / 200 seat (72%)". Push interval: **3h** *(chốt 14/07 — xem §6.9.9 `usage_push_interval_hours`)*.
...
| Renew | subscription.renewed + previous_subscription_id | (như Submit + previous). **`package_version` giữ nguyên của sub cũ — kể cả khi gói đó đã `RETIRED`** (D1 grandfathering). |
| Force-sync usage | usage.force_sync_requested | correlation_id |

### 6.9.7. Section 6 — Inbound Event Mapping

> **Đồng bộ 16/07/2026 — EDR đã xác nhận (`_event_catalog.md` §3, nguồn sự thật).** Điểm cốt lõi: **`license.expired` = tín hiệu VÀO ân hạn** → mirror `GRACE` (KHÔNG dùng `license.grace_started`); mọi gói EDR đều đi qua ân hạn. Trong ba giai đoạn ân hạn/hạn chế, `sub.status` **giữ ACTIVE**; chỉ → SUSPENDED khi `license.suspended`. Tạm dừng/Khôi phục xác nhận qua **`license.suspended` / `license.reactivated` mang `correlation_id`** (bỏ hoàn toàn `tenant.*`).
...
| license.pending | LICENSE_MIRROR=PENDING + timeline |
| license.provisioning | LICENSE_MIRROR=PROVISIONING + timeline |
| license.activated | LICENSE_MIRROR=ACTIVE + capture activation_date/expiration_date → sync sub.activated_at=activation_date, sub.end_date=expiration_date + sub→ACTIVE + timeline |
| license.expiring_soon | LICENSE_MIRROR=EXPIRING_SOON (sub giữ ACTIVE) + timeline |
| license.expired | LICENSE_MIRROR=GRACE (sub giữ ACTIVE — **vào ân hạn**, khách vẫn dùng đủ) + timeline |
| license.grace_expired | LICENSE_MIRROR=RESTRICTED (sub giữ ACTIVE — hạn chế tính năng 30 ngày; OQ-INT-08) + timeline |
| license.suspended | LICENSE_MIRROR=SUSPENDED + sub→SUSPENDED. **2 nguồn:** CÓ `correlation_id` → do CRM ra lệnh Tạm dừng (§3.3); KHÔNG có → tự nhiên (hết ân hạn không gia hạn) |
| license.reactivated | LICENSE_MIRROR=ACTIVE + sub→ACTIVE — **luôn CÓ** `correlation_id` (phản hồi lệnh Khôi phục, §3.3) |
| license.revoked | LICENSE_MIRROR=REVOKED + sub→REVOKED *(⚠️ EDR Phase 1 chưa gửi thật — §3.4)* |
| license.renewed | LICENSE_MIRROR=RENEWED + sub→RENEWED (nếu có successor ACTIVE) |
| license.usage_synced | LICENSE_MIRROR.usage_snapshot update |
| subscription.command_failed | Timeline "Sản phẩm từ chối lệnh" + alert (§4.4) |

### 6.9.8. Section 7 — Provisioning Timeline Display Templates

| Event | Template VN |
|----|----|
| subscription.submitted (outbound) | "Đã gửi yêu cầu xuống EDR" |
...
| license.activated | "License đã active (hiệu lực {data.activation_date} → {data.expiration_date})" |
| license.expiring_soon | "License sắp hết hạn ({data.expiration_date})" |
| license.expired | "License vào giai đoạn ân hạn, KH vẫn dùng đủ tính năng" |
| license.grace_expired | "Hết ân hạn — KH chuyển sang dùng hạn chế tính năng (30 ngày)" |
| license.suspended | "License đã ngừng bên EDR" |
| license.reactivated | "License đã khôi phục theo lệnh CRM" |
| license.revoked | "License đã bị thu hồi" |
...
| **Input** | webhook event EDR (`event_key`, `payload`, `correlation_id`, `subscription_id`, `product_id`, `event_id`, `occurred_at`) — metadata chuẩn theo `_event_catalog.md` §2 |
| **Output** | Không có (side-effect: cập nhật license_mirror / timeline / sub transition) |
| **Business Rules** | BR-02.1: Lookup `event_key` trong mappingBR-02.2: Unknown → category="unknown" + warning, không rejectBR-02.3: Apply actions theo thứ tự: license_mirror → timeline → sub transition → notificationBR-02.4: Idempotent qua `event_id`BR-02.5: Validate `payload` schema; schema fail → mark webhook FAILEDBR-02.6: Chặn sự kiện đến muộn — chỉ áp dụng khi `occurred_at` mới hơn `license_mirror.last_event_occurred_at` (§5.2) |
| **User Stories** | US-02: Là hệ thống, tôi map webhook EDR thành cập nhật mirror/timeline/sub chuẩn xác |
| **AC** | AC-02.1: license.activated → license_mirror updated, sub → ACTIVE, timeline appendAC-02.2: license.expired → license_mirror=GRACE, sub giữ ACTIVEAC-02.3: unknown.event → category="unknown" + warningAC-02.4: license.activated thiếu activation_date → FAILED + audit error |
| **API** |  |
...
|----|----|
| **Actor** | System (called by M-07) |
| **Input** | `event_key`, `subscription_id` |
| **Output** | event (metadata §2 + payload) |
| **Business Rules** | BR-03.1: Lookup sub + contract + customer + packageBR-03.2: Build theo schema §6.9.6 (Outbound Mapping)BR-03.3: Gắn `subscription_id` + `product_id`; `correlation_id = event_id` của lệnh (để product echo lại khi phản hồi, §2.1)BR-03.4: Sanitize PII không cầnBR-03.5: Versioning payload qua `schema_version` |
| **User Stories** | US-03: Là hệ thống, tôi build payload outbound đúng schema để EDR pickup xử lý |
| **AC** | AC-03.1: Build subscription.submitted → đầy đủ customer, package, scopeAC-03.2: Build subscription.suspended → có correlation_id + reasonAC-03.3: schema_version hiển thị |
| **API** |  |
...
{"event_type": "license.usage_synced", "data": {"seats_used": 145, "seats_total": 200}}
...
3.  EDR consumer pickup → tính usage mới → push webhook license.usage_synced
...
6.  Webhook về: license.activated cho sub-002, license.renewed cho sub-001
...
2.  Webhook license.expired về CRM
...
| ~~**OQ-INT-01**~~ | ~~Tạm dừng/Khôi phục xác nhận qua tenant.* hay license.*?~~ → ✅ **ĐÃ CHỐT 16/07 (EDR xác nhận):** qua **`license.suspended`/`license.reactivated` mang `correlation_id`** (command-ack) — **bỏ hoàn toàn `tenant.*`**. Chi tiết: UC-INT-02 BR-12, §6.7.5 BR-04.9. | BA + Team EDR | — | ✅ Đóng 16/07 |
| ~~**OQ-INT-08**~~ | ~~Khi mirror = RESTRICTED (hạn chế tính năng) thì `sub.status` sang EXPIRED hay giữ ACTIVE?~~ → ✅ **ĐÃ CHỐT 16/07:** `sub.status` **giữ ACTIVE** (sub chưa chết; hạn chế 30 ngày phản ánh qua mirror). Chi tiết: UC-INT-02 AC-10, §6.9.7. | BA + Team EDR | — | ✅ Đóng 16/07 |
| **OQ-INT-05b** | Tin inbox lỗ/chưa xử lý **cũ ngoài cửa sổ 3 ngày** có nên luôn hiện (bất kể tuổi) không, hay để filter thời gian ẩn? *(hiện: PA2 lọc thuần thời gian, tab Kênh bắt việc kẹt cũ — UC-INT-05 BR-09)* | BA | Thấp | Rà khi vận hành thật |
...
| Command-ack (Xác nhận lệnh) | Tin inbound (`license.suspended`/`license.reactivated`) mang `correlation_id` khớp `event_id` lệnh CRM đã gửi → xác nhận sản phẩm đã thực thi lệnh; vắng `correlation_id` = thay đổi tự nhiên (M-07) |
| occurred_at | Thời điểm sự việc **thật sự xảy ra ở nguồn** (product) — nền của **chống đến muộn** & **xử lý lại theo chuỗi**; khác `received_at` (lúc CRM nhận) |
| Đã ngừng tự gửi lại (DEAD_LETTER) | Lệnh outbox đã thử tự gửi đủ số lần (10) mà không được → **dừng tự động**, chờ vận hành gửi lại tay (UC-INT-04) |
| Xử lý lại theo chuỗi (Chained replay) | Xử lý lại tin inbox lỗi bằng cách chạy lại **CẢ chuỗi** sự kiện của cùng subscription theo `occurred_at` — không xử lý lẻ (tránh dựng lại lịch sử sai; UC-INT-05) |
| SG-4 | Quyết định bảo mật: tin **sai chữ ký → trả 4xx và KHÔNG ghi `WEBHOOK_INBOX`** (chỉ log hạ tầng) → chống DoS/phình bảng, mọi dòng inbox đều đã xác thực |
...
| Correlation ID | **= `event_id` của lệnh CRM** mà product echo lại để nối lệnh (outbound) ↔ phản hồi (inbound command-ack). **KHÔNG phải `subscription_id`** *(SG-3, chốt 16/07)* |
