...
[6.3.2. FR-SUB-01: Create Subscription [103](#fr-sub-01-create-subscription)](#fr-sub-01-create-subscription)

[6.3.3. FR-SUB-02: Update Subscription [103](#fr-sub-02-update-subscription)](#fr-sub-02-update-subscription)

[6.3.4. FR-SUB-03: Cancel DRAFT Subscription [104](#fr-sub-03-cancel-draft-subscription)](#fr-sub-03-cancel-draft-subscription)

[6.3.5. FR-SUB-04: Submit Subscription [104](#fr-sub-04-submit-subscription)](#fr-sub-04-submit-subscription)

[6.3.6. FR-SUB-05: View Subscription Detail [105](#fr-sub-05-view-subscription-detail)](#fr-sub-05-view-subscription-detail)

[6.3.7. FR-SUB-06: Search & Filter Subscriptions [105](#fr-sub-06-search-filter-subscriptions)](#fr-sub-06-search-filter-subscriptions)

[6.3.8. FR-SUB-07: Suspend Subscription [106](#fr-sub-07-suspend-subscription)](#fr-sub-07-suspend-subscription)

[6.3.9. FR-SUB-08: Resume Subscription [107](#fr-sub-08-resume-subscription)](#fr-sub-08-resume-subscription)

[6.3.10. FR-SUB-09: Revoke Subscription [107](#fr-sub-09-revoke-subscription)](#fr-sub-09-revoke-subscription)

[6.3.11. FR-SUB-10: Create Successor Subscription (Renew / Upgrade / Downgrade) [108](#fr-sub-10-create-successor-subscription-renew-upgrade-downgrade)](#fr-sub-10-create-successor-subscription-renew-upgrade-downgrade)

[6.3.12. FR-SUB-11: Auto-transition on License State Change [108](#fr-sub-11-auto-transition-on-license-state-change)](#fr-sub-11-auto-transition-on-license-state-change)
...
| CONTRACT ─ CONTRACT_POOL_ITEM | 1 : 0..N | Chỉ type=RESELLER có pool item. DIRECT không có. Mỗi dòng = 1 product |
...
| product_id | UUID FK | ✓ | PRODUCT; UNIQUE per (contract_id, product_id) |
...
| id | UUID | ✓ | PK |
| product_id | UUID FK | ✓ |  |
| code | string | ✓ | Unique trong product |
| version | int | ✓ | Versioning catalog |
...
| 6 | Pool License | contract.poolItems | ❌ | Chỉ render nội dung với RESELLER. Mỗi pool item = 1 dòng: {productName}: {allocatedQty}/{poolTotal}. Với DIRECT: hiển thị "—". |
...
| **AC ID** | **Given** | **When** | **Then** |
|----|----|----|----|
| AC-CON-03-04 | Sales chọn Reseller, thêm 1 pool: EPP Business, 500 license | Nhấn "Tạo" | Contract tạo thành công với type = RESELLER, poolItems = \[{productId: EPP, pool_total: 500, allocated_qty: 0}\]. |
...
| AC-CON-03-05 | HĐ RESELLER có 2 pool item | Mở tab Thông tin HĐ | Accordion Pool License hiển thị 2 dòng pool: {productName}: {allocatedQty}/{poolTotal}. Progress bar màu cam nếu > 80%. |
...
| 1i | System | Thêm pool item mới vào contract.poolItems: {productId, poolTotal, allocatedQty: 0}. pool_total trở thành cố định (không thể sửa lại) ngay sau bước này (BR-03). |
...
| **AC ID** | **Given** | **When** | **Then** |
|----|----|----|----|
| AC-CON-04-08 | Hợp đồng RESELLER "HD-CMC-2026-020" hiện chưa có pool item nào | Sales nhấn "＋ Thêm Pool", chọn EPP Business, nhập 300 | Pool item mới thêm vào poolItems: {productId: EPP, poolTotal: 300, allocatedQty: 0}. Toast "Đã thêm Pool — EPP Business: 300 seats". |
...
### 6.3.2. FR-SUB-01: Create Subscription

| Mô tả | Tạo subscription mới. |
|----|----|
| **Actor** | Sales |
| **Input** | contract_id, package_id, start_date, end_date, seat_count, scope (jsonb), deal_code, metadata |
| **Output** | sub_id, status=DRAFT, HTTP 201 |
| **Business Rules** | BR-01.1: Contract tồn tại + customer ACTIVE (không BLACKLIST)BR-01.2: Package ACTIVE (không RETIRED)BR-01.3: start_date ≤ end_date; end_date ≤ today + 10 nămBR-01.4: scope validate theo schema Product ModuleBR-01.5: Tạo luôn ở DRAFT |
| **User Stories** | US-01: Là Sales, tôi muốn tạo sub DRAFT để review trước khi submit |
| **AC** | AC-01.1: Data hợp lệ → status=DRAFTAC-01.2: Package RETIRED → reject 400AC-01.3: KH BLACKLIST → reject 403AC-01.4: Scope sai schema → reject 400 SCOPE_VALIDATION_FAILED |
| **API** |  |

### 6.3.3. FR-SUB-02: Update Subscription

| Mô tả | Cập nhật subscription. |
|----|----|
| **Actor** | Sales (mình), Manager (any) |
| **Input** | subscription_id + fields |
| **Output** | sub (đã cập nhật), HTTP 200 |
| **Business Rules** | BR-02.1: DRAFT sửa mọi fieldBR-02.2: PENDING_PROVISION không sửaBR-02.3: ACTIVE/SUSPENDED chỉ sửa deal_code, metadata, noteBR-02.4: Terminal không sửaBR-02.5: Audit before/after |
| **User Stories** | US-02: Là Sales, tôi muốn sửa sub DRAFT trước khi submit, hoặc sửa metadata khi đã ACTIVE |
| **AC** | AC-02.1: Update DRAFT package → OKAC-02.2: Update ACTIVE deal_code → OKAC-02.3: Update ACTIVE package → reject 400 IMMUTABLE_IN_ACTIVEAC-02.4: Update PENDING_PROVISION → reject 400 SUB_IN_FLIGHT |
| **API** |  |

### 6.3.4. FR-SUB-03: Cancel DRAFT Subscription

| Mô tả | Hủy subscription draft. |
|----|----|
| **Actor** | Sales (mình), Manager (any) |
| **Input** | subscription_id |
| **Output** | HTTP 204 (đã xoá) |
| **Business Rules** | BR-03.1: Chỉ state=DRAFTBR-03.2: Sales của mình; Manager anyBR-03.3: Hard delete + audit |
| **User Stories** | US-03: Là Sales, tôi muốn huỷ sub nháp tạo nhầm khi chưa submit |
| **AC** | AC-03.1: Cancel DRAFT của mình → successAC-03.2: State ≠ DRAFT → reject 400AC-03.3: Sales xoá DRAFT người khác → reject 403 |
| **API** |  |

### 6.3.5. FR-SUB-04: Submit Subscription

| Mô tả | DRAFT → PENDING_PROVISION + publish event xuống broker. |
|----|----|
| **Actor** | Sales (mình), Manager (any) |
| **Input** | subscription_id |
| **Output** |  sub.status=PENDING_PROVISION, HTTP 200 |
| **Business Rules** | BR-04.1: Chỉ DRAFTBR-04.2: Pre-submit validate lạiBR-04.3: Transaction:<br>sub.status = PENDING_PROVISIONTạo LICENSE_MIRROR (status=PENDING)OUTBOX_EVENT subscription.created với payload đầy đủAppend PROVISIONING_EVENT "Sub submitted to product"Audit log<br>BR-04.4: Worker async pickup outbox (M-07)BR-04.5: Sau submit không sửa package/dates/scope |
| **User Stories** | US-04: Là Sales, tôi muốn submit sub xuống product và theo dõi tiến độ |
| **AC** | AC-03.1: Cancel DRAFT của mình → successAC-03.2: State ≠ DRAFT → reject 400AC-03.3: Sales xoá DRAFT người khác → reject 403 |
| **API** |  |

### 6.3.6. FR-SUB-05: View Subscription Detail

| Mô tả | Xem chi tiết subscription. |
|----|----|
| **Actor** | Sales (mình), Manager (any) |
| **Input** | subscription_id |
| **Output** | Sub info, Customer/Contract brief, Package brief, LICENSE_MIRROR (status, usage), PROVISIONING_TIMELINE, Sub Chain (previous + successors), AUDIT_LOG recent |
| **Business Rules** | BR-05.1: Chain trace đầy đủBR-05.2: P95 ≤ 1s |
| **User Stories** | US-05: Là CSKH, tôi muốn xem detail trả lời "KH đang dùng gì, đến đâu, hạn nào"US-06: Là Manager, tôi muốn xem chain hiểu lịch sử KHUS-06b: Là Sales, tôi muốn xem detail để phân tích cơ hội bán hàng (cross-sell, up-sell) |
| **AC** | AC-05.1: 5 timeline event → đầy đủAC-05.2: Chain 3 đời → hiển thị đúngAC-05.3: Sub không tồn tại → 404 |
| **API** |  |

### 6.3.7. FR-SUB-06: Search & Filter Subscriptions

| Mô tả | Tìm/lọc subscription theo KH, HĐ, package, product, trạng thái, ngày — gồm dashboard sub sắp hết hạn. |
|----|----|
| **Actor** | Sales, CSKH, Manager |
| **Input** | filter (customer_id, contract_id, package_id, product_id, status, start_date_range, end_date_range), sort (default created_at desc; có end_date asc cho renewal dashboard) |
| **Output** | list subscription + total |
| **Business Rules** | BR-06.1: Index: status, customer_id, contract_id, package_id, end_dateBR-06.2: Default exclude terminalBR-06.3: P95 ≤ 500ms với 50K sub |
| **User Stories** | US-07: Là Sales, tôi muốn lọc sub sắp hết hạn (sort end_date) để lên kế hoạch renewal |
| **AC** | AC-06.1: Filter ACTIVE + EDR → đúng listAC-06.2: Sort end_date asc → sub gần hết hạn lên đầu |
| **API** |  |

### 6.3.8. FR-SUB-07: Suspend Subscription

| Mô tả | Tạm dừng sub đang ACTIVE (non-revenue action) — publish event cho product, không revert nếu product fail. |
|----|----|
| **Actor** | Sales, Manager |
| **Input** | sub_id + reason (req) + metadata |
| **Output** | sub.status=SUSPENDED, HTTP 200 |
| **Business Rules** | BR-07.1: Chỉ state=ACTIVEBR-07.2: Transaction:<br>sub.status = SUSPENDEDOUTBOX_EVENT subscription.suspended (reason in payload)Audit log<br>BR-07.3: License mirror chưa đổi đến khi product webhook về. Gap acceptable trong ngưỡng SLA mismatch_sla_minutes (mặc định 15 phút, configurable per product). Quá ngưỡng mà mirror chưa SUSPENDED → alert ops (FR-LIC-07). CRM không revert sub.status nếu product fail; Sales/CSKH thấy lệch qua UI badge "Đang đồng bộ" / "Lệch trạng thái"BR-07.4: Notification Sales/CSKH |
| **User Stories** | US-08: Là Sales, tôi muốn suspend khi KH yêu cầu pause dịch vụ |
| **AC** | AC-07.1: Suspend ACTIVE → SUSPENDED, outbox ghiAC-07.2: Suspend non-ACTIVE → reject 400AC-07.3: Không reason → reject 400 |
| **API** |  |

### 6.3.9. FR-SUB-08: Resume Subscription

| Mô tả | Mở lại sub đang SUSPENDED về ACTIVE (non-revenue action), publish event cho product. |
|----|----|
| **Actor** | Sales, Manager |
| **Input** | sub_id + reason |
| **Output** | sub.status=ACTIVE, HTTP 200 |
| **Business Rules** | BR-08.1: Chỉ SUSPENDEDBR-08.2: Transaction: status=ACTIVE + OUTBOX subscription.resumed + AuditBR-08.3: License mirror cập nhật qua webhook |
| **User Stories** | US-09: Là Sales, tôi muốn mở lại sub đã tạm dừng khi KH tiếp tục dịch vụ |
| **AC** | AC-08.1: Resume SUSPENDED → ACTIVEAC-08.2: Non-SUSPENDED → reject 400 |
| **API** |  |

### 6.3.10. FR-SUB-09: Revoke Subscription

| Mô tả | Thu hồi sub (terminal, không resume) — vd khi KH vi phạm hợp đồng. |
|----|----|
| **Actor** | Manager, License Admin |
| **Input** | sub_id + reason (req) |
| **Output** | sub.status=REVOKED, HTTP 200 |
| **Business Rules** | BR-09.1: ACTIVE/SUSPENDED/EXPIRED → REVOKEDBR-09.2: Transaction: status=REVOKED + OUTBOX subscription.revoked + AuditBR-09.3: Terminal, không resume |
| **User Stories** | US-10: Là Manager, tôi muốn revoke khi KH vi phạm contract |
| **AC** | AC-09.1: Revoke ACTIVE → REVOKEDAC-09.2: DRAFT/PENDING/RENEWED → reject 400AC-09.3: Sales (non-Manager) → reject 403AC-09.4: Không reason → reject 400 |
| **API** |  |

### 6.3.11. FR-SUB-10: Create Successor Subscription (Renew / Upgrade / Downgrade)

| Mô tả | Tạo sub mới kế thừa sub cũ. |
|----|----|
| **Actor** | Sales |
| **Input** | predecessor_subscription_id, type (RENEWAL/UPGRADE/DOWNGRADE), new_package_id, new_start_date, new_end_date, new_scope, new_seat_count, metadata |
| **Output** | sub mới status=DRAFT, HTTP 201 |
| **Business Rules** | BR-10.1: Predecessor ACTIVE/SUSPENDED/EXPIRED (không REVOKED/RENEWED)BR-10.2: Sub mới DRAFT, previous_subscription_id = predecessor_idBR-10.3: Sales review/sửa DRAFT → FR-SUB-04 SubmitBR-10.4: Predecessor → RENEWED chỉ khi sub mới ACTIVE (qua FR-SUB-11 webhook license RENEWED)BR-10.5: Validate type vs tier (Phase 1: type=UPGRADE/DOWNGRADE → reject 501 NOT_IMPLEMENTED chờ OQ-G)BR-10.6: 1 predecessor max 1 successor "đang active" (DRAFT/PENDING/ACTIVE) |
| **User Stories** | US-07: Là Sales, tôi muốn renew sub sắp hết hạn → tạo sub mới cùng package |
| **AC** | AC-10.1: RENEWAL → sub mới DRAFT, link previousAC-10.2: type=UPGRADE/DOWNGRADE Phase 1 → reject 501 NOT_IMPLEMENTEDAC-10.3: Predecessor REVOKED → reject 400AC-10.4: Successor đã có active → reject 409 SUCCESSOR_ALREADY_EXISTS |
| **API** |  |

### 6.3.12. FR-SUB-11: Auto-transition on License State Change
...
2 cấp catalog: Product (admin đăng ký 1 lần) + Package (Sales tạo theo schema Product Module cho phép). Package có versioning: sửa khi đã có sub → tạo version mới + retire cũ.

### 6.4.2. FR-CAT-01: Register Product into Catalog

| Mô tả | Admin đăng ký 1 product vào catalog (1 lần) — sau đó Sales tự tạo Package theo schema Product Module. |
|----|----|
| **Actor** | License Admin, Dev |
| **Input** | code (unique, immutable), name, integration_type (immutable), has_instances (immutable), module_class_id, runtime_config (jsonb) |
| **Output** | product_id, webhook_secret (return 1 lần), HTTP 201 |
| **Business Rules** | BR-01.1: code unique, immutableBR-01.2: module_class_id validate tồn tại trong codebaseBR-01.3: Sau register, M-07 enable consumer + webhook handlerBR-01.4: Webhook secret auto generate, return 1 lần, Admin lưu để cấu hình product backendBR-01.5: Audit với approver |
| **User Stories** | US-01: Là Admin, tôi muốn đăng ký product 1 lần — sau đó Sales tự tạo Package |
| **AC** | AC-01.1: Register code mới + module hợp lệ → successAC-01.2: Code trùng → reject 409AC-01.3: Module không tồn tại → reject 400 MODULE_NOT_DEPLOYED |
| **API** |  |

### 6.4.3. FR-CAT-02: Update Product Runtime Config

| Mô tả | Sửa runtime config của product (notification thresholds, rotate webhook secret...); các field định danh immutable. |
|----|----|
| **Actor** | License Admin |
| **Input** | product_id + runtime_config fields |
| **Output** | product (config đã cập nhật), HTTP 200 |
| **Business Rules** | BR-02.1: code, integration_type, has_instances IMMUTABLEBR-02.2: module_class_id chỉ đổi khi không có sub đang ACTIVE/PENDINGBR-02.3: Webhook secret rotation: old valid 7 ngày graceBR-02.4: Audit log |
| **User Stories** | US-04: Là Admin, tôi muốn rotate webhook secret an toàn (grace 7 ngày) không gián đoạn product |
| **AC** | AC-02.1: Rotate secret → success, old valid 7 daysAC-02.2: Update integration_type → reject 400 IMMUTABLEAC-02.3: Update notification_thresholds → áp dụng cho sub mới + đang active |
| **API** |  |

### 6.4.4. FR-CAT-03: View Product Detail

| Mô tả | Xem chi tiết product — schema module, runtime config (mask secret cho non-Admin), thống kê package/sub. |
|----|----|
| **Actor** | Sales, License Admin |
| **Input** | product_id |
| **Output** | Product info, Module schemas, Runtime config (secret mask cho non-Admin), Stats packages + subs |
| **Business Rules** | BR-03.1: Sales chỉ schema-related; secret + sensitive chỉ Admin |
| **User Stories** | US-03: Là Sales, tôi muốn xem schema product để biết cách cấu hình Package |
| **AC** | AC-03.1: Admin → full + secretAC-03.2: Sales → schema only, secret mask |
| **API** |  |

### 6.4.5. FR-CAT-04: List Products

| Mô tả | Liệt kê product trong catalog kèm thống kê (cached). |
|----|----|
| **Actor** | Sales, CSKH, Manager, License Admin |
| **Input** | filter (status), pagination |
| **Output** | list product + stats (số package, số sub) |
| **Business Rules** | BR-04.1: Default ACTIVEBR-04.2: Stats cached 60s |
| **User Stories** | US-04: Là Sales, tôi muốn xem danh sách product đang ACTIVE để chọn khi tạo Package |
| **AC** | AC-04.1: List default → chỉ product ACTIVE kèm stats |
| **API** |  |

### 6.4.6. FR-CAT-05: Create Package

| Mô tả | Sales tạo Package mới (DRAFT) theo schema Product Module cho phép; Publish để mở bán. |
|----|----|
| **Actor** | Sales |
| **Input** | product_id, code, name, duration_months, default_seat_count, default_instance_count, features, rules, price, scope_template, metadata |
| **Output** | package_id, status=DRAFT, version=1 |
| **Business Rules** | BR-05.1: product_id ACTIVEBR-05.2: code unique trong scope productBR-05.3: Schema validate theo Product Module declareBR-05.4: Luôn tạo DRAFT, Publish (FR-CAT-09) để mở bánBR-05.5: version = 1; tăng khi tạo version mớiBR-05.6: Audit |
| **User Stories** | US-05: Là Sales, tôi muốn tạo Package để bán hàng |
| **AC** | AC-05.1: Tạo EDR Package schema hợp lệ → DRAFTAC-05.2: Trùng code → reject 409AC-05.3: Schema sai → reject 400 SCHEMA_VALIDATION_FAILED chi tiếtAC-05.4: Product RETIRED → reject 400 |
| **API** |  |

### 6.4.7. FR-CAT-06: Update Package

| Mô tả | Sửa Package — DRAFT sửa in-place; ACTIVE đã có sub thì tạo version mới (versioning). |
|----|----|
| **Actor** | Sales |
| **Input** | package_id + fields |
| **Output** | package (DRAFT in-place) hoặc new_package_id (version mới) |
| **Business Rules** | BR-06.1: DRAFT sửa in-placeBR-06.2: ACTIVE chưa có sub: in-place, audit warningBR-06.3: ACTIVE có sub: tạo version mới, sửa version mới rồi Publish; version cũ tự RETIRED khi PublishBR-06.4: Schema validateBR-06.5: Audit |
| **User Stories** | US-06: Là Sales, tôi muốn cập nhật giá Package mới không ảnh hưởng KH cũ — qua versioning |
| **AC** | AC-06.1: Update DRAFT → in-placeAC-06.2: Update ACTIVE có sub → trả new_package_id version mớiAC-06.3: Update RETIRED → reject 400 |
| **API** |  |

### 6.4.8. FR-CAT-07: View Package Detail

| Mô tả | Xem chi tiết package — thông tin, product, thống kê sub, lịch sử version. |
|----|----|
| **Actor** | Sales, CSKH, Manager |
| **Input** | package_id |
| **Output** | Package info, Product brief, Stats subs, Version history |
| **Business Rules** | BR-07.1: View không cần quyền đặc biệtBR-07.2: P95 ≤ 500ms |
| **User Stories** | US-07: Là Sales, tôi muốn xem chi tiết package + lịch sử version trước khi tạo sub |
| **AC** | AC-07.1: Package nhiều version → hiển thị version history đúng |
| **API** |  |

### 6.4.9. FR-CAT-08: Search & List Packages

| Mô tả | Tìm/lọc package theo product, trạng thái, tier. |
|----|----|
| **Actor** | Sales, CSKH, Manager |
| **Input** | filter (product_id, status, tier), search (code, name) |
| **Output** | list package + total |
| **Business Rules** | BR-08.1: Default chỉ ACTIVEBR-08.2: Index: product_id, status, codeBR-08.3: P95 ≤ 500ms |
| **User Stories** | US-08: Là Sales, tôi muốn lọc package ACTIVE theo product để chọn khi bán |
| **AC** | AC-08.1: Filter product + ACTIVE → đúng list |
| **API** |  |

### 6.4.10. FR-CAT-09: Publish / Retire Package

| Mô tả | Mở bán (Publish DRAFT → ACTIVE) hoặc ngừng bán (Retire ACTIVE → RETIRED) package. |
|----|----|
| **Actor** | Sales (Publish DRAFT mình); Manager (any + Retire) |
| **Input** | package_id |
| **Output** | package (status mới), HTTP 200 |
| **Business Rules** | BR-09.1: Publish (DRAFT → ACTIVE): Sales author hoặc ManagerBR-09.2: Retire (ACTIVE → RETIRED): Manager onlyBR-09.3: RETIRED không cho phép Sales chọn tạo sub mớiBR-09.4: RETIRED không ảnh hưởng sub đã cóBR-09.5: Publish version mới → version cũ auto-RETIREBR-09.6: Audit log |
| **User Stories** | US-09: Là Sales, tôi muốn publish package nháp để mở bán; Manager retire khi ngừng bán |
| **AC** | AC-09.1: Publish DRAFT → ACTIVEAC-09.2: Retire ACTIVE → RETIREDAC-09.3: Publish non-DRAFT → reject 400AC-09.4: Sales retire → reject 403AC-09.5: Publish v2 → v1 auto-RETIRE |
| **API** |  |
...
| Field | Type | Required | Mô tả |
|----|----|----|----|
| tier | enum | ✓ | STANDARD / PRO / ENTERPRISE |
| tenant_mode | enum | ✓ | SINGLE / MULTI |
| default_seat_count | int | ✓ |  |
| duration_months | int | ✓ | 12 / 24 / 36 |
| features | array | ✓ | Feature flags |
| scope_template.tenant_mode_options | array | optional |  |
| price | decimal | optional |  |

**Validation**:

- tier=ENTERPRISE mới dùng tenant_mode=MULTI

- default_seat_count ≥ 5 cho ENTERPRISE
...
|----|----|----|
| Submit | subscription.created | customer, contract_code, package, scope, dates, correlation_id |
...
| Renew (RENEWAL) | subscription.created + context.type="RENEWAL" + previous_subscription_id | (như Submit + previous) |
...
| **Input** | package fields EDR (tier, tenant_mode, default_seat_count, duration_months, features...) |
| **Output** | pass / fail + danh sách lỗi per field |
| **Business Rules** | BR-01.1: Required fields cóBR-01.2: Enum validateBR-01.3: Cross-field:<br>tier=ENTERPRISE mới tenant_mode=MULTIdefault_seat_count ≥ 5 cho ENTERPRISE<br>BR-01.4: duration_months ∈ {12,24,36}BR-01.5: Error chi tiết per field |
| **User Stories** | US-01: Là hệ thống, tôi validate schema package EDR để Sales không tạo cấu hình sai |
| **AC** | AC-01.1: Package valid → passAC-01.2: Thiếu tier → fail REQUIREDAC-01.3: MULTI+STANDARD → fail INVALID_TIER_MODE_COMBOAC-01.4: duration=6 → fail INVALID_DURATION |
| **API** |  |
...
| Renew EDR sub (FR-SUB-10 RENEWAL)                       | ✓         |
...
**Step 4 — Sales tạo subscription DRAFT (FR-SUB-01)**:
...
**Step 5 — Sales submit sub (FR-SUB-04)**:
...
1.  Sales suspend (FR-SUB-07) với reason
...
3.  KH đồng ý → Sales tạo successor (FR-SUB-10 type=RENEWAL):
