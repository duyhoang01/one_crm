...
> ⚠️ **Toàn bộ §3 là mô hình ĐỀ XUẤT của BA, CHƯA xác nhận với đội EDR** — sẽ chốt trong buổi làm việc (xem OQ-INT-01/08/10, §9).
...
| **LH-2** | Không có event nào đưa license vào ân hạn; `license.expired` bị hiểu là "đã chết" | Khách **đang dùng bình thường** trong ân hạn bị báo **"đang mất dịch vụ" mức Cao** | §3.1, §3.2 — ⚠️ **đề xuất:** `license.expired` là tín hiệu **vào ân hạn** → mirror `GRACE` ("Trong ân hạn"), `sub` giữ ACTIVE; kế tiếp `license.grace_expired` → `RESTRICTED`. Không hiểu `license.expired` là "đã chết". |
