...
Đây là **danh mục event tích hợp (Event Catalog)** — bản khai báo **nội dung** của mọi event trao đổi giữa OneCRM và các Product Module (EDR, C-Shield, AV, CA). Nó là **nguồn sự thật duy nhất** trả lời 4 câu hỏi:
...
4. **Sản phẩm mới khi tích hợp phải khai báo tối thiểu những gì?** — 4 nhóm event bắt buộc cho mọi Product Module (§6).

> **Phân vai với FRS khác:** [UC-INT-01](UC-INT-01_publish_outbound_event.md)/[UC-INT-02](UC-INT-02_receive_inbound_event.md) đặc tả *cơ chế* (làm sao publish/nhận, retry, outbox/inbox); tài liệu này khai báo *nội dung* (gửi/nhận **cái gì**, mang **trường nào**, nghĩa **là gì**). Đây là **hợp đồng kỹ thuật** hai bên phải cùng tuân thủ — không phải mô tả một chức năng có giao diện.

Tài liệu **thay thế** `docs/prd/inbound_outbound_concept.md` v1.0 (bản cũ có 3 lỗ hổng làm hỏng M-05 — xem §7).
...
| **Đội sản phẩm (EDR, C-Shield, AV…)** | Biết CRM **gửi lệnh gì** và **mong nhận phản hồi gì**, đúng format nào — để hai bên **thống nhất hợp đồng event** trước khi tích hợp |
| **Dev / BA M-05 (License Mirror & Usage)** | Bảng ánh xạ event → `license_mirror.status` (§3.2) **trực tiếp** quyết định trạng thái M-05 hiển thị; các việc kéo theo ở §8 |
| **QA** | Cơ sở viết test: idempotency (§5.1), chống event đến muộn (§5.2), mapping trạng thái đúng (§3.2), lệnh thất bại (§4.4) |
| **BA / PO** | Theo dõi Open Questions (§9), quyết các gap giữa demo và PRD (vd CMC CA — §6.2), phê duyệt thay đổi nghiệp vụ (GRACE / REVOKED — §3) |
...
| **OQ** | Open Question — câu hỏi mở chưa chốt (§9) |
| **LH** | Lỗ hổng của tài liệu tích hợp cũ (§7) |
...
| # | Trường | Kiểu dữ liệu | Bắt buộc | Mô tả | Hệ quả nếu thiếu |
|---|---|---|---|---|---|
| 1 | `event_id` | UUID | Có | Mã định danh **duy nhất** của từng event, do bên gửi sinh ra. Là khoá để chống xử lý trùng. | Không thể phát hiện event bị gửi lặp. Vì cơ chế giao nhận đảm bảo *"ít nhất một lần"* (at-least-once), một event chắc chắn sẽ có lúc đến hai lần; thiếu `event_id`, CRM sẽ áp dụng nó hai lần (ví dụ: trừ pool license hai lần). |
| 2 | `event_key` | string | Có | Mã định danh **loại** event, theo quy ước `nhóm.hành_động` (ví dụ: `license.activated`). Quyết định cách CRM diễn giải payload và ánh xạ trạng thái. | Không xác định được event thuộc loại nào để xử lý; toàn bộ bảng ánh xạ tại §3 và §4 trở nên vô hiệu. |
| 3 | `occurred_at` | timestamp (UTC, ISO-8601) | Có | Thời điểm sự việc **thật sự xảy ra tại nguồn** — không phải thời điểm gửi tin đi. Là mốc để sắp xếp thứ tự các event. | Không thể chặn event đến sai thứ tự (§5.2). Một event cũ đến muộn sẽ ghi đè lên trạng thái mới hơn, khiến license bị sai trạng thái vĩnh viễn. |
| 4 | `subscription_id` | string | Có | Mã subscription trong CRM mà event liên quan tới. Là khoá đối chiếu event về đúng đối tượng nghiệp vụ. | Event trở thành *"mồ côi"* — CRM nhận được nhưng không biết thuộc khách hàng / subscription nào để áp dụng. |
| 5 | `product_id` | string | Có | Mã Product Module gửi hoặc nhận event (ví dụ: `CMC_EDR`). | Không chọn được đúng khoá bảo mật để xác thực (§5.3) và đúng bảng ánh xạ trạng thái của sản phẩm đó. |
| 6 | `correlation_id` | UUID | Có với event outbound; với event inbound chỉ bắt buộc khi đó là phản hồi cho một lệnh của CRM | Mã liên kết một lệnh CRM gửi đi với phản hồi tương ứng của sản phẩm. Với một lệnh outbound, `correlation_id` chính bằng `event_id` của lệnh đó; sản phẩm phản hồi bằng cách đặt lại giá trị này vào event inbound. | Không phân biệt được nguyên nhân của một thay đổi. Ví dụ: không trả lời được câu hỏi *"license bị tạm dừng vì CRM ra lệnh, hay vì nó tự hết hạn?"* (§3.2, §3.3). |
| 7 | `schema_version` | string | Có | Phiên bản cấu trúc của `payload`, cho phép nâng cấp định dạng payload theo thời gian mà vẫn giữ tương thích ngược. | Khi cấu trúc payload thay đổi, bên nhận không biết đang đọc theo phiên bản nào, dẫn tới vỡ tương thích. |
| — | `payload` | object | Có | Phần **nội dung nghiệp vụ** riêng của từng loại event; cấu trúc phụ thuộc `event_key` (chi tiết tại §3.2, §4.2, §4.3). *(Đây là lớp payload, liệt kê ở đây để thấy quan hệ với metadata.)* | Không có nội dung để xử lý nghiệp vụ (ví dụ: không có ngày hết hạn để cập nhật, không có số ghế để đối chiếu mức dùng). |
...
### 3.1 Vòng đời license của EDR — mô hình đã xác nhận với EDR *(16/07/2026)*

**Đội EDR đã xác nhận:** **mọi gói EDR đều đi qua ân hạn** trước khi ngừng — không có gói nào "chết ngay". Chuỗi trạng thái tự nhiên (khi khách không gia hạn) đi qua **ba giai đoạn**, trong đó `sub.status` **giữ nguyên ACTIVE** cho tới khi ngừng hẳn:
...
> ✅ **Điểm cốt lõi EDR xác nhận (16/07) — khác giả định 14/07:**
...
> - `license.grace_expired` → mirror **RESTRICTED** (hạn chế tính năng, 30 ngày); `sub` **vẫn giữ ACTIVE** *(OQ-INT-08 chốt 16/07: sub chưa chết, chỉ mirror phản ánh mức hạn chế)*.
...
| 4 | `license.expired` ✅ *(EDR xác nhận 16/07: gửi khi VÀO ân hạn — KHÔNG phải "đã chết")* | **Hết hạn nhưng vào ân hạn.** Khách **vẫn dùng đủ tính năng**. | `GRACE` | **Trong ân hạn** | `sub` giữ **ACTIVE**. → view **Cơ hội doanh thu: "Gia hạn gấp"** |
| 5 | `license.grace_expired` ✅ | **Hết ân hạn** → khách chuyển sang **dùng hạn chế tính năng, trong 30 ngày** | `RESTRICTED` | **Hạn chế tính năng** | `sub` giữ **ACTIVE** *(OQ-INT-08 chốt 16/07: sub chưa chết; mirror RESTRICTED thể hiện mức hạn chế)*. → **Cơ hội gia hạn KHẨN, xếp trên cả "Gia hạn gấp"** |
...
> **Lưu ý mô hình (16/07):** `sub.status` chỉ đi thẳng `ACTIVE → SUSPENDED` trong vòng đời tự nhiên của EDR — **không** dùng trạng thái `EXPIRED` ở cấp sub cho luồng này (ba giai đoạn ân hạn/hạn chế đều để `sub` ACTIVE, mức độ suy giảm phản ánh qua `license_mirror.status`).

### 3.3 Xác nhận lệnh Tạm dừng/Khôi phục — event `license.*` mang `correlation_id` (command-ack) *(EDR xác nhận 16/07/2026)*
...
> ✅ **OQ-INT-01 — đã chốt (EDR xác nhận 16/07):** Tạm dừng/Khôi phục xác nhận qua `license.suspended` / `license.reactivated` mang `correlation_id`; **bỏ hoàn toàn `tenant.*`** khỏi danh mục.
...
**Không bịa event cho sản phẩm chưa làm việc với ta.** Nhưng mọi Product Module **bắt buộc** khai báo đủ **4 nhóm** dưới đây vào `PRODUCT_MODULE_CONFIG.inbound_event_mapping` / `outbound_event_mapping`, đều **kèm metadata chung §2**:
...
| **N4. Báo lệnh thất bại** | ❌ **PRD chưa khai báo** — đề xuất thêm `subscription.command_failed` cùng cấu trúc §4.4 *(nhất quán với EDR, tránh lặp lại LH-3 cho 2 sản phẩm này)* | ❌ **PRD chưa khai báo** — đề xuất tương tự |

⚠️ **Lưu ý khi đối chiếu:** tên event trong PRD **không theo convention `license.*`** đã chốt cho EDR (§3, §NT-5) — đây là điều **cần đội sản phẩm C-Shield/AV xác nhận lại** khi vào Phase 3/4, không phải điều FRS này tự đổi tên được. Cũng chưa có event **vào-ân-hạn** tương đương cho C-Shield dù PRD có trạng thái `GRACE_PERIOD` ở LICENSE_INSTANCE (§6.10.6) — cùng dạng lỗ hổng LH-2 của EDR, **áp dụng luôn nguyên tắc NT-2** khi Phase 3 triển khai thật: mọi đổi trạng thái phải có event tương ứng, không suy diễn.

### 6.2 CMC CA — không có gì để khai báo, kể cả skeleton
...
| **LH-2** | Không có event nào đưa license vào ân hạn; `license.expired` bị hiểu là "đã chết" | Khách **đang dùng bình thường** trong ân hạn bị báo **"đang mất dịch vụ" mức Cao** | §3.1, §3.2 — ✅ **EDR xác nhận (16/07):** chính `license.expired` là tín hiệu **vào ân hạn** → mirror `GRACE` ("Trong ân hạn"), `sub` giữ ACTIVE; kế tiếp `license.grace_expired` → `RESTRICTED`. Không còn hiểu `license.expired` là "đã chết". |
...
| Mã | Nội dung | Mức |
|---|---|---|
| ~~OQ-INT-01~~ | ✅ **Đã chốt (EDR xác nhận 16/07):** Tạm dừng/Khôi phục xác nhận qua `license.suspended` / `license.reactivated` mang `correlation_id` (§3.3) — **bỏ hoàn toàn `tenant.*`**; Thu hồi dùng `license.revoked` riêng (§3.4). | — |
...
| ~~OQ-INT-10~~ 🆕 | ✅ **Đã chốt (EDR xác nhận 16/07):** EDR gửi `license.expired` để báo **vào ân hạn** → mirror `GRACE`; **không dùng `license.grace_started`**. **Mọi gói EDR đều đi qua ân hạn** (§3.1, §3.2). | — |
| **OQ-INT-02** | C-Shield / AV phản hồi thế nào cho `subscription.suspended` / `terminated` / `renewed`? **Đã có khung đề xuất** ở §6.1 (nguồn PRD §6.10.7/§6.11.7), nhưng event key **chưa theo convention `license.*`** và N4 (báo lỗi) **PRD chưa khai báo** — cần đội sản phẩm C-Shield/AV xác nhận, không phải BA CRM tự quyết. | 🔴 Chặn dev *(Phase 3/4)* |
...
| ~~OQ-INT-08~~ | ✅ **Đã chốt (16/07):** khi mirror = `RESTRICTED` (hạn chế tính năng), `sub.status` **giữ ACTIVE** (PA A) — sub chưa chết; mức hạn chế phản ánh qua `license_mirror.status`, không đổi trạng thái sub (§3.2 #5). | — |
