...
| **Trigger** | Tab **Sự kiện nhận về**; hoặc từ kết luận *"CRM xử lý lỗi"* ở UC-INT-03; hoặc từ UC-INT-06. |
...
| **BR-09** | Mặc định **ẩn** tin `license.usage_synced` **đã xử lý thành công**; có bộ lọc bật lại. | Chúng chiếm **>95% khối lượng** (8 tin/sub/ngày) và **không mang thông tin gì khi thành công** — để hiện thì chúng chôn mất mọi sự kiện quan trọng. ⚠️ *(OQ-INT-07 — chờ duyệt)* |
| **BR-10** | Bộ lọc: trạng thái · sản phẩm · loại sự kiện · mã subscription · khoảng thời gian. Phân trang **15 dòng/trang**. | — |
...
| **2** | System | Render danh sách: trạng thái · **nội dung sản phẩm báo về** *(tên nghiệp vụ tiếng Việt)* · khách hàng + mã subscription *(hoặc nhãn "không xác định được subscription")* · sản phẩm · **xác thực chữ ký** · **lỗi ở bước nào** · nhận lúc. Ẩn `usage_synced` đã xử lý thành công (BR-09). |
...
| **AF-02c** | System | **Cho xem trước** (BR-04): chuỗi gồm **{n} sự kiện**, liệt kê theo thứ tự, và **trạng thái license sau khi xong**. Ghi rõ: *"Đây là thao tác kỹ thuật nội bộ — **không gửi thông báo cho khách hàng**, **không phát sinh lệnh mới** sang sản phẩm."* (BR-05) |
...
| Bước | Actor | Hành động / Phản hồi |
|---|---|---|
| **AF-03a** | License Admin | Bấm **"Xử lý lại {n} tin lỗi"**. |
...
| Mã | Given | When | Then |
|---|---|---|---|
| AC-INT-05-09 | AV có **42 tin lỗi** thuộc **3 subscription**. | Bấm "Xử lý lại 42 tin lỗi". | Nhóm thành **3 chuỗi**; mỗi chuỗi chạy **tuần tự bên trong**, 3 chuỗi chạy **song song** (BR-07). |
| AC-INT-05-10 | Có **620 tin lỗi**. | Bấm xử lý lại hàng loạt. | Xử lý **500**, **báo rõ**: *"còn 120 tin — chạy lại lượt sau"* (BR-07). |

### Nhóm 4: Nhiễu

| Mã | Given | When | Then |
|---|---|---|---|
| AC-INT-05-11 | Trong 24h qua có **4.000 tin `usage_synced` đã xử lý thành công** và **3 tin lỗi**. | Mở màn. | Mặc định **chỉ thấy 3 tin lỗi** + các sự kiện quan trọng; 4.000 tin usage thành công **được ẩn**, bật lại bằng bộ lọc (BR-09). |
...
- ✅ Bảng không bị rác usage che mất sự kiện quan trọng
- ⚠️ **OQ-INT-05** *(🔴)*: BR-03/BR-08 phụ thuộc `occurred_at` trong envelope + cột `last_event_occurred_at` — chờ duyệt
- ⚠️ **OQ-INT-07** *(🟠)*: BR-09 ẩn mặc định `usage_synced` thành công — chờ duyệt
