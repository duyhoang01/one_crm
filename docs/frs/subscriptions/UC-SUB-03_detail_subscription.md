...
| **Liên kết** | UC-SUB-01 (Danh sách), UC-SUB-02 (Tạo mới), UC-SUB-04 (Chỉnh sửa), UC-SUB-05 (Xác nhận), UC-SUB-06 (Xóa Draft), UC-SUB-07 (Gia hạn), UC-SUB-08 (Thu hồi / Tạm dừng / Khôi phục) |
...
|---|---|---|---|
| 1 | Sales / Manager / ... | Click vào row subscription từ màn hình UC-SUB-01, hoặc được điều hướng tự động sau khi tạo thành công ở UC-SUB-02. | — |
| 2 | System | Kiểm tra subscription tồn tại trong hệ thống.<br>[Gateway — Sub tồn tại?]<br>→ [Có]: Tiếp tục bước 3.<br>→ [Không]: → **EF-01**. | — |
| 3 | System | Render trang chi tiết: breadcrumb ("Subscription › {sub.id}"), banner DRAFT (nếu `sub.status = DRAFT`, BR-11), hero section (ID badge, status badge, tên khách hàng, action buttons theo role và status, BR-03 → BR-10), tab bar 4 tab (BR-01). | BR-02, BR-11 |
| 4 | System | Render nội dung tab mặc định "Thông tin chung" (`sub-tp-info`): các trường thông tin, link HĐ, link KH, trạng thái License Mirror, Sub Chain (nếu có). | BR-02, BR-12, BR-13 |
| 5 | User | (Tuỳ chọn) Click tab khác trên tab bar. | → **AF-01**, **AF-02**, **AF-03** |
| 6 | User | (Tuỳ chọn) Click một action button trong hero section. | → **AF-04** hoặc điều hướng sang UC tương ứng (BR-04 → BR-10). |
| 7 | User | Click "← Quay lại" hoặc breadcrumb "Subscription". | System quay về màn hình UC-SUB-01. |

### 2.2 Luồng phụ

#### AF-01 — Click tab "License"

| Bước | Tác nhân | Hành động |
|---|---|---|
| 1 | User | Click tab "License" trên tab bar. |
| 2 | System | Gọi `subRenderLicense(sub)`. Render nội dung `sub-tp-license` theo trạng thái: (a) `sub.status = DRAFT` hoặc `licMirrorStatus = N/A` → placeholder "Chưa có thông tin license"; (b) `licMirrorStatus = PENDING` → placeholder "Đang chờ kích hoạt"; (c) Các trạng thái khác → card "Thông tin License" (badge trạng thái, ngày kích hoạt, ngày hết hạn), guidance banner (GRACE/SUSPENDED/EXPIRED/REVOKED), quota/bar chart (DIRECT: seats; RESELLER: license đang active), metadata từ sản phẩm (nếu có). |
| 3 | System | Kiểm tra mismatch: nếu `sub.status ≠ license_mirror.status` (GRACE không tính là mismatch) và lệch > 15 phút SLA → hiển thị warning banner mismatch. |

#### AF-02 — Click tab "Timeline"

| Bước | Tác nhân | Hành động |
|---|---|---|
| 1 | User | Click tab "Timeline" trên tab bar. |
| 2 | System | Gọi `subRenderTimeline(sub)`. Render danh sách sự kiện theo thứ tự thời gian giảm dần (mới nhất lên đầu). Mỗi sự kiện hiển thị: icon màu theo loại sự kiện, tên sự kiện, thời gian (dd/MM/yyyy HH:mm), actor, chi tiết. |

#### AF-03 — Click tab "Audit"

| Bước | Tác nhân | Hành động |
|---|---|---|
| 1 | User | Click tab "Audit" trên tab bar. |
| 2 | System | Gọi `subRenderAuditLog(sub)`. Render bảng audit log: Thời gian | Nguồn | Actor | Hành động | Chi tiết | Kết quả. |

#### AF-04 — Click link Hợp đồng

| Bước | Tác nhân | Hành động |
|---|---|---|
| 1 | User | Click link mã hợp đồng trong tab Thông tin chung. |
| 2 | System | Gọi `showModule('contract')`. Mở UC-CON-03 với hợp đồng tương ứng. |

#### AF-05 — Click link Khách hàng

| Bước | Tác nhân | Hành động |
|---|---|---|
| 1 | User | Click link tên khách hàng trong tab Thông tin chung. |
| 2 | System | Gọi `showModule('customer')`. Mở UC-CUS-03 với khách hàng tương ứng. |

### 2.3 Luồng ngoại lệ

#### EF-01 — Subscription không tồn tại

| Bước | Tác nhân | Hành động |
|---|---|---|
| 1 | System | Hiển thị toast error: "Subscription không tồn tại". |
| 2 | System | Giữ nguyên màn hình hiện tại (không điều hướng sang trang chi tiết). |
...
| AC-SUB-03-13 | Sub thuộc HĐ "HĐ-2024-001", KH "FPT Software". | Click link "HĐ-2024-001" trong tab Thông tin chung. | Hệ thống mở UC-CON-03 của hợp đồng đó (AF-04). |
...
| Mã | Given | When | Then |
|---|---|---|---|
| AC-SUB-03-21 | Đang ở trang chi tiết sub "SUB-2026-001". | Click "← Quay lại". | Quay về màn hình UC-SUB-01 (danh sách subscription). |
| AC-SUB-03-22 | Đang ở trang chi tiết sub "SUB-2026-001". | Click breadcrumb "Subscription". | Quay về màn hình UC-SUB-01. |
| AC-SUB-03-23 | Sub DIRECT, `status = DRAFT`. Sales click ✅ Xác nhận. | Click nút ✅ Xác nhận. | Điều hướng sang UC-SUB-05 với sub hiện tại. |

### Nhóm 7: Exception

| Mã | Given | When | Then |
|---|---|---|---|
| AC-SUB-03-24 | Sub "SUB-2026-999" không tồn tại trong hệ thống. | Hệ thống cố điều hướng đến trang chi tiết SUB-2026-999. | Toast error hiển thị "Subscription không tồn tại". Màn hình hiện tại (UC-SUB-01) được giữ nguyên. Không điều hướng sang trang chi tiết. |
...
- ✅ Luồng phụ đã định nghĩa: AF-01 (tab License), AF-02 (tab Timeline), AF-03 (tab Audit), AF-04 (link HĐ), AF-05 (link KH)
- ✅ Luồng ngoại lệ đã xử lý: EF-01 (sub không tồn tại)
- ✅ BR đã định nghĩa rõ (BR-01 đến BR-13)
- ✅ AC bao phủ 7 nhóm, 24 AC — vượt ngưỡng tối thiểu 15
