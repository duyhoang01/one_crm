...
| **BR-02** | **Phân quyền view**: view **Cần xử lý**, dải KPI "License cần xử lý" và banner sự cố diện rộng **chỉ hiển thị** cho Manager và License Admin (quyền `license:view_issues`). Sales/CSKH truy cập trực tiếp → tự chuyển về view **Cơ hội doanh thu**. |
...
| **BR-08** | **Khách hàng triển khai tại chỗ (on-premise)**: **vẫn hiện** trong danh sách (để Sales không tưởng dữ liệu bị mất), gắn nhãn **"Không theo dõi tự động"**; **không có** mức sử dụng / thời điểm cập nhật; **loại khỏi mọi cảnh báo** và khỏi cơ hội bán thêm. |
| **BR-09** | **Sales phụ trách** hiển thị tại view Cơ hội doanh thu, lấy từ **người tạo subscription**. *(Mô hình phân công khách hàng nằm ngoài phạm vi giai đoạn này.)* |
...
| **4** | System | Render view đang chọn. |
| **5** | Người dùng | [Điểm quyết định — Người dùng muốn làm gì?]<br>→ Chọn: đổi view · lọc/sắp xếp · mở chi tiết một license · bấm hành động trên dòng. |

### 2.2 Luồng phụ

**[AF-01: Đổi view]** — tại bước 5.

| Bước | Actor | Hành động / Phản hồi |
|---|---|---|
| **5a** | Người dùng | Bấm tab **Cần xử lý** / **Cơ hội doanh thu** / **Tất cả license**. |
| **5b** | System | Render view tương ứng. Số đếm badge và KPI **không đổi** (đã tính ở bước 2). |
| → | — | Ở lại màn hình. |

**[AF-02: Lọc / tìm kiếm / sắp xếp — view Tất cả license]** — tại bước 5.

| Bước | Actor | Hành động / Phản hồi |
|---|---|---|
| **5c** | Người dùng | Nhập từ khoá (tên KH, mã subscription, mã định danh) hoặc chọn filter: Sản phẩm · Trạng thái license · Mức sử dụng · Thời hạn. Hoặc bấm header cột để sắp xếp. |
| **5d** | System | Lọc theo **BR-06**, sắp xếp, phân trang lại từ trang 1, cập nhật dòng đếm. |
| → | — | Ở lại màn hình. |

**[AF-03: Mở chi tiết license]** — tại bước 5.

| Bước | Actor | Hành động / Phản hồi |
|---|---|---|
| **5e** | Người dùng | Nhấn vào một dòng bất kỳ (ở cả 3 view). |
| → | — | Mở **UC-LIC-02**. Kết thúc tại **End 2**. |

**[AF-04: Hành động trên dòng cơ hội]** — tại bước 5, chỉ view Cơ hội doanh thu.

| Bước | Actor | Hành động / Phản hồi |
|---|---|---|
| **5f** | Sales | Bấm **Gia hạn** trên dòng cơ hội loại *Gia hạn*. |
| → | — | Mở modal gia hạn (**UC-SUB-07**). Kết thúc tại **End 3**. |

### 2.3 Luồng ngoại lệ

**[EF-01: Không có bản ghi]** — tại bước 4.

| Bước | Actor | Hành động / Phản hồi |
|---|---|---|
| **4a** | System | View **Cần xử lý** rỗng → empty state "Không có license nào cần xử lý". View **Cơ hội** rỗng → "Chưa có cơ hội nào". View **Tất cả** rỗng sau lọc → "Không có license nào khớp bộ lọc". |
| → | — | Ở lại màn hình. |
...
|---|---|---|---|---|---|---|
| 1 | License cần xử lý | Số việc trong hàng đợi | "{n} việc mức độ cao" | Đỏ | View Cần xử lý | **Ẩn** với Sales / CSKH / Auditor (BR-02) |
| 2 | Khách hàng sắp dùng hết | Số cơ hội bán thêm | "Nên chào bán thêm số lượng" | Cam | View Cơ hội | Mọi vai trò |
| 3 | Khách hàng sắp hết hạn | Số cơ hội gia hạn | "Trong 60 ngày tới — cần chốt gia hạn" | Xanh dương | View Cơ hội | Mọi vai trò |
...
| 6 | Căn cứ | *Gia hạn:* "Còn {n} ngày là hết hạn" hoặc "Đã quá hạn {n} ngày" (đỏ).<br>*Bán thêm:* "Đã dùng {used}/{total} {đơn vị} ({pct}%)" + thanh tiến độ.<br>**Nếu số liệu cũ:** thêm dòng ⚠ *"Số liệu chưa xác minh — sản phẩm chưa cập nhật {X}"* (BR-12) |
| 7 | Hết hạn | Ngày hết hạn. **Đỏ** khi ≤ 30 ngày hoặc đã quá hạn |
| 8 | Nên làm gì | Câu hướng dẫn. Với đại lý còn pool: *"Pool của đại lý còn {n} — cấp thêm từ pool, chưa phát sinh doanh thu mới"* (BR-13) |
...
| AC-LIC-01-14 | Có 2 license đã hết hạn (ngày hết hạn trong quá khứ) và 3 license Active hết hạn trong 25 ngày. | Chọn filter "Hết hạn trong 30 ngày". | Chỉ trả về **3 license còn hiệu lực**; 2 license đã hết hạn **không** xuất hiện (BR-06). |
